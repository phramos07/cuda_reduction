...
__global__ void reducaoSoma(int *v, int *v_reduzido) {
    ...
    // Thread 0 soma os elementos 0 e 1
    // Thread 1 não faz nada
    // Thread 2 soma os elementos 2 e 3
    // Thread 3 não faz nada --> divergência de threads
	for (int passo = 1; passo < blockDim.x; passo *= 2) {
        // A cada ciclo, metade das threads do ciclo anterior
        // estarão realizando trabalho.
        // Ao final somente 1 thread escreve em somas_parciais.
		if (threadIdx.x % (2 * passo) == 0) {
